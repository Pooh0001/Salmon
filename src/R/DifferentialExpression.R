...
"line_plot" <- function(dds=dds,vst=vst,gene_id=gene_id){
    ...
    p <- ggplot(bind_cols(as.data.frame(colData(dds)),
                ...
                aes(x=Time,y=value,col=Treatment,group=Treatment)) +
...
dds$Treatment <- relevel(droplevels(dds$Treatment),"Mock")
...
vst <- assay(vsd)
vst <- vst - min(vst)
dir.create(here("data/analysis/DE_LamV_vs_Mock"),showWarnings=FALSE)
save(vst,file=here("data/analysis/DE_LamV_vs_Mock/vst-aware.rda"))
write_delim(as.data.frame(vst) %>% rownames_to_column("ID"),
            here("data/analysis/DE_LamV_vs_Mock/vst-aware.tsv"))
...
#' ### LamV vs Mock at 24h
lamVAt24 <- extract_results(dds=dds,vst=vst,contrast="Treatment_LamV_vs_Mock",
                             default_dir=here("data/analysis/DE_LamV_vs_Mock"),
                             default_prefix="LamV_vs_Mock_at_24-",
                             labels=dds$Treatment,
                             sample_sel=dds$Time=="24")

#' ### Compare Time 72 vs 48
Time72vs48inMock <- extract_results(dds=dds,vst=vst,contrast=list("Time_72_vs_24","Time_48_vs_24"),
                            default_dir=here("data/analysis/DE_LamV_vs_Mock"),
                            default_prefix="DE_Mock_T72_vs_48-",
                            labels=paste(dds$Time),
                            sample_sel=dds$Time %in% c("48","72") & dds$Treatment == "Mock")

#' ### LamV vs Mock at 48h (specific)
lamVAt48 <- extract_results(dds=dds,vst=vst,contrast="TreatmentLamV.Time48",
                            default_dir=here("data/analysis/DE_LamV_vs_Mock"),
                            default_prefix="LamV_at_48-",
                            labels=dds$Treatment,
                            sample_sel=dds$Time=="48")

dev.null <- lapply(lamVAt48$all,line_plot,dds=dds,vst=vst)

#' ### LamV at 48h (all)
lamV_vs_Mock_At48 <- extract_results(dds=dds,vst=vst,contrast=c(0,1,0,0,1,0),
                            default_dir=here("data/analysis/DE_LamV_vs_Mock"),
                            default_prefix="LamV_vs_Mock_at_48-",
                            labels=dds$Treatment,
                            sample_sel=dds$Time=="48")

#' ### Mock at 48h (specific)
MockAt48 <- extract_results(dds=dds,vst=vst,contrast="Time_48_vs_24",
                            default_dir=here("data/analysis/DE_LamV_vs_Mock"),
                            default_prefix="Mock_48_vs_24-",
                            labels=dds$Time,
                            sample_sel=dds$Time %in% c("24","48") & dds$Treatment == "Mock")
...
res.list=list(Mock=MockAt48,
         LamV=lamV_vs_Mock_At48,
         intx=lamVAt48)
...
grid.draw(venn.diagram(lapply(res.list,"[[","all"),
                      NULL,
                      fill=pal[1:length(res.list)]))

dev.null <- lapply(
    setdiff(intersect(MockAt48$all,lamV_vs_Mock_At48$all),lamVAt48$all),
